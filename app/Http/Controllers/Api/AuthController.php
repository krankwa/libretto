...
class AuthController extends Controller
{
    ...
    /**
     * Get all user tokens (for token management)
     */
    public function tokens(Request $request)
    {
        ...
        $tokens = DB::table('personal_access_tokens')
            ->where('tokenable_type', get_class($user))
            ->where('tokenable_id', $user->id)
            ->select(['id', 'name', 'abilities', 'expires_at', 'created_at', 'last_used_at'])
            ->get();
        ...
        return response()->json([
            'tokens' => $tokens->map(function ($token) {
                return [
                    ...
                    'last_used_at' => $token->last_used_at ? Carbon::parse($token->last_used_at)->toISOString() : null,
                    'is_expired' => $token->expires_at ? Carbon::now()->greaterThan(Carbon::parse($token->expires_at)) : false
                ];
            })
        ]);
    }
